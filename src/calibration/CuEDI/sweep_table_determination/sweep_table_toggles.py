...
class sweepToggles:
    ...
    feed_back_gain_ratio = 500 # 500:1 for OCHRE, 1000:1 for ACES-II

    add_linear_bool = True # add a number of linear points near the bottom of the sweep
    N_linear = 5 # number of linear points to add


    def construct_sweep_table_from_DAC_codes(self,**kwargs):

    def construct_sweep_table_from_Emax(self,**kwargs):
        sweep_energies = []
        ...
        # DAC Value
        sweep_DAC_codes = np.round(self.digitize(sweep_voltages / self.feed_back_gain_ratio))
    ...
    def plot_sweep(self,energies,errors,voltages,DAC_codes,steps):

        # plot everything
        fig, ax = plt.subplots(3, sharex=True)

        # Energies
        ax[0].scatter(steps, energies)
        ax[0].errorbar(steps, energies, yerr=errors, capsize=5)
        ax[0].set_ylabel('Energy (Estimate) [eV]')
        # ax[0].set_ylim(0,14000)

        # Voltages
        ax[1].scatter(steps, voltages)
        ax[1].set_ylabel('Inner Hemisphere Voltage [V]')
        # ax[1].set_ylim(0,1400)

        # DAC Value
        ax[2].scatter(steps, DAC_codes)
        ...
        for i in range(3):
            ax[i].grid()
            ax[i].set_xticks([i + 1 for i in range(len(steps))])
            # ax.set_yscale('log')
            if i == 2:
